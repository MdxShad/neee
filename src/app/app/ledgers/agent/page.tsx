...
export default async function AgentLedgerPage() {
  ...
  return (
    <div className="space-y-6">
      <div><h1 className="text-2xl font-semibold">Agent Payment Ledger</h1></div>
                ...
                <div key={l.id} className="rounded-md border border-zinc-200 p-3 space-y-3">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div>
                      ...
                      <div className="text-xs text-zinc-500">Agent: {l.agent.name} • {l.admission.course.name}</div>
                    </div>
                    <div className="text-sm">Commission {formatINR(l.commissionAmount)} • Paid {formatINR(l.amountPaid)} • Pending {formatINR(pending)} • {l.status === 'PAID' ? <Badge variant="success">Paid</Badge> : <Badge variant="warning">Pending</Badge>}</div>
                  </div>

                  <form id={formId} action={addAgentPaymentAction.bind(null, l.id)} className="grid gap-2 md:grid-cols-6">
                    ...
                    <div><FileUploader inputName="proofUrl" pathPrefix="payments/agent" accept=".jpg,.jpeg,.png,.pdf" label="Upload proof" /></div>
